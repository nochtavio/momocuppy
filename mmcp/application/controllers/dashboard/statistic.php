...
class statistic extends CI_Controller {
  ...
  function index() {
    ...
    //JS
    $content['js'][0] = 'js/dashboard/private/subscriber.js';

    //Modal
    $content['modal'][0] = $this->load->view('dashboard/subscriber/modal_add', '', TRUE);
    $content['modal'][1] = $this->load->view('dashboard/subscriber/modal_edit', '', TRUE);
    $content['modal'][2] = $this->load->view('dashboard/subscriber/modal_remove', '', TRUE);

    $data['content'] = $this->load->view('dashboard/subscriber/index', $content, TRUE);
  ...
  function show_object() {
    ...
    if ($checkadmin > 0) {
      ...
      $totalrow = $this->model_subscriber->get_object(0, $email, $active, $order)->num_rows();
      ...
      if ($totalrow > 0) {
        $query = $this->model_subscriber->get_object(0, $email, $active, $order, $limit, $size)->result();
        $temp = 0;
  ...
  function edit_object() {
    ...
    if ($checkadmin > 0) {
      ...
      $data['result'] = "s";
      $this->model_subscriber->edit_object($id, $email);
  ...
  function get_object() {
    ...
    if ($checkadmin > 0) {
      $id = $this->input->post('id', TRUE);
      $query = $this->model_subscriber->get_object($id)->result();
      foreach ($query as $row) {
        $data['result'] = "s";
  ...
  function set_active() {
    ...
    if ($checkadmin > 0) {
      ...
      $query = $this->model_subscriber->get_active($id)->result();
      foreach ($query as $row) {
        $active = $row->active;
      }
      if ($active === "0") {
      ...
      $data['result'] = "s";
      $data['active'] = $active;
      $this->model_subscriber->set_active($id, $active);
  ...
  function remove_object() {
    ...
    if ($checkadmin > 0) {
      ...
      $data['result'] = "s";
      $this->model_subscriber->remove_object($id);
