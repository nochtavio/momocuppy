...
class detail_product_img extends CI_Controller {
  ...
  function index() {
    ...
    //JS
    $content['js'][0] = 'js/dashboard/private/detail_product_img.js';

    //Modal
    $content['modal'][0] = $this->load->view('dashboard/detail_product_img/modal_add', $content, TRUE);
    $content['modal'][1] = $this->load->view('dashboard/detail_product_img/modal_edit', '', TRUE);
    $content['modal'][2] = $this->load->view('dashboard/detail_product_img/modal_remove', '', TRUE);

    $data['content'] = $this->load->view('dashboard/detail_product_img/index', $content, TRUE);
  ...
  function show_object() {
    ...
    if ($checkadmin > 0) {
      ...
      $totalrow = $this->model_detail_product_img->get_object(0, $id_product, $visible, $order)->num_rows();
      ...
      if ($totalrow > 0) {
        $query = $this->model_detail_product_img->get_object(0, $id_product, $visible, $order, $limit, $size)->result();
        $temp = 0;
  ...
  function add_object() {
    ...
    if ($checkadmin > 0) {
      ...
      if ($uplst) {
        $data['result'] = "s";
        $this->model_detail_product_img->add_object($id_product, $saved_img);
      } else {
        $data['result'] = "f";
      }
  ...
  function edit_object() {
    ...
    if ($checkadmin > 0) {
      ...
      if ($uplst > 0) {
        $data['result'] = "s";
        $this->model_detail_product_img->edit_object($id, $saved_img);
      } else {
        $data['result'] = "f";
      }
  ...
  function get_object() {
    ...
    if ($checkadmin > 0) {
      $id = $this->input->post('id', TRUE);
      $query = $this->model_detail_product_img->get_object($id)->result();
      foreach ($query as $row) {
        $data['result'] = "s";
  ...
  function set_visible() {
    ...
    if ($checkadmin > 0) {
      ...
      $query = $this->model_detail_product_img->get_visible($id)->result();
      foreach ($query as $row) {
        $visible = $row->visible;
      }
      if ($visible === "0") {
      ...
      $this->model_detail_product_img->set_visible($id, $visible);
  ...
  function remove_object() {
    ...
    if ($checkadmin > 0) {
      $id = $this->input->post('id', TRUE);
      //Get Image File
      $query_product_img = $this->model_detail_product_img->get_img_file($id)->result();
      ...
      $data['result'] = "s";
      $this->model_detail_product_img->remove_object($id);
