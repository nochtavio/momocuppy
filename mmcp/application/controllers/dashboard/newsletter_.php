...
class newsletter extends CI_Controller {
  ...
  function index() {
    ...
    //Modal
    $content['modal'][0] = $this->load->view('dashboard/newsletter/modal_add', $content, TRUE);
  ...
  function show_object() {
    ...
    if ($checkadmin > 0) {
      ...
      $totalrow = $this->model_newsletter->get_object(0, $visible, $order)->num_rows();
      ...
      if ($totalrow > 0) {
        $query = $this->model_newsletter->get_object(0, $visible, $order, $limit, $size)->result();
        $temp = 0;
  ...
  function add_object() {
    ...
    if ($checkadmin > 0) {
      ...
      if ($uplst) {
        $data['result'] = "s";
        $this->model_newsletter->add_object($id_product, $saved_img);
      } else {
        $data['result'] = "f";
      }
  ...
  function edit_object() {
    ...
    if ($checkadmin > 0) {
      ...
      $id_product = $this->model_newsletter->get_id_product($id)->row()->id_product;
      ...
      if ($uplst > 0) {
        $data['result'] = "s";
        $this->model_newsletter->edit_object($id, $saved_img);
      } else {
        $data['result'] = "f";
      }
  ...
  function remove_object() {
    ...
    if ($checkadmin > 0) {
      $id = $this->input->post('id', TRUE);
      //Get Image File
      $query_product_img = $this->model_newsletter->get_img_file($id)->result();
